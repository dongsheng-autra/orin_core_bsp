...
int gmsl_max9295_setup_streaming(struct device *dev)
{
	struct max9295 *priv = dev_get_drvdata(dev);
	int err = 0;
	u32 csi_mode;
	...
	struct map_ctx map_pipe_dtype[] = {
		{GMSL_CSI_DT_YUV422, MAX9295_PIPE_Z_DT_ADDR, 0x1E,
			MAX9295_ST_ID_2},
...
EXPORT_SYMBOL(gmsl_max9295_setup_streaming);

int gmsl_max9295_setup_control(struct device *dev)
{
	struct max9295 *priv = dev_get_drvdata(dev);
	int err = 0;
	struct gmsl_link_ctx *g_ctx;
...
EXPORT_SYMBOL(gmsl_max9295_setup_control);

int gmsl_max9295_reset_control(struct device *dev)
{
	struct max9295 *priv = dev_get_drvdata(dev);
	int err = 0;
...
EXPORT_SYMBOL(gmsl_max9295_reset_control);

int gmsl_max9295_sdev_pair(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
	struct max9295 *priv;
	int err = 0;
...
EXPORT_SYMBOL(gmsl_max9295_sdev_pair);

int gmsl_max9295_sdev_unpair(struct device *dev, struct device *s_dev)
{
...
EXPORT_SYMBOL(gmsl_max9295_sdev_unpair);
...
MODULE_DESCRIPTION("GMSL Serializer driver gmsl_max9295");
