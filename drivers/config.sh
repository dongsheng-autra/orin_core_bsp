...
i2ctransfer -f -y 30 w3@0x6b 0x04 0x0b 0x00  # MIPI CSI disable
i2ctransfer -f -y 30 w3@0x6b 0x00 0x06 0xFF  # Enable all 4 Links in GMSL2 mode

sleep 0.1

# [ISX031] 3Gbps [IMX490 & OX08B] 6Gbps
if [ ${camera_array[key]} == sg3-isx031-gmsl2 ]; then
    i2ctransfer -f -y 30 w3@0x6b 0x00 0x10 0x11  # MAX96717F use 3Gbps
    i2ctransfer -f -y 30 w3@0x6b 0x00 0x11 0x11
    green_print "DPHY Speed 3Gbps"
else
    i2ctransfer -f -y 30 w3@0x6b 0x00 0x10 0x22  # MAX9295 use 6Gbps
...
i2ctransfer -f -y 30 w3@0x6b 0x00 0xF0 0x62  # LinkB -> Pipe Z -> Pipe 1, LinkA -> Pipe Z -> Pipe 0
i2ctransfer -f -y 30 w3@0x6b 0x00 0xF1 0xEA  # LinkD -> Pipe Z -> Pipe 3, LinkC -> Pipe Z -> Pipe 2
i2ctransfer -f -y 30 w3@0x6b 0x00 0xF4 0x0F  # Enable 0 - 3 Pipes

i2ctransfer -f -y 30 w3@0x6b 0x09 0x0B 0x07  # why MAP_SRC_0 MAP_SRC_1 MAP_SRC_2, maybe means 3 fifo ?
i2ctransfer -f -y 30 w3@0x6b 0x09 0x2D 0x15  # Map CSI to Controller 1 from MAX96712 datasheet
i2ctransfer -f -y 30 w3@0x6b 0x09 0x0D 0x1E  # YUV422 - 0x1E
i2ctransfer -f -y 30 w3@0x6b 0x09 0x0E 0x1E  # Map to VC0
...
i2ctransfer -f -y 30 w3@0x6b 0x09 0x4E 0x5E  # Map to VC1 maybe it's should be equal to ORIN vc-id?
...
i2ctransfer -f -y 30 w3@0x6b 0x09 0x8E 0x9E  # Map to VC2
...
i2ctransfer -f -y 30 w3@0x6b 0x09 0xCE 0xDE  # Map to VC3
...
i2ctransfer -f -y 30 w3@0x6b 0x08 0xA0 0x04  # default MIPI PHY 2x4 lanes
i2ctransfer -f -y 30 w3@0x6b 0x08 0xA3 0xE4  # PORTA 4 lanes
i2ctransfer -f -y 30 w3@0x6b 0x08 0xA4 0xE4  # PORTB 4 lines

i2ctransfer -f -y 30 w3@0x6b 0x09 0x0A 0xC0  # default Four data lanes
...
i2ctransfer -f -y 30 w3@0x6b 0x08 0xA2 0xF0  # Eable all MIPI PHYS
...
i2ctransfer -f -y 30 w3@0x6b 0x00 0x06 0xF1 # Enable LinkA
...
i2ctransfer -f -y 30 w3@0x6b 0x00 0x06 0xF2 # Enable LinkB
...
i2ctransfer -f -y 30 w3@0x6b 0x00 0x06 0xF4 # Enable LinkC
...
i2ctransfer -f -y 30 w3@0x6b 0x00 0x06 0xF8 # Enable LinkD
...
sleep 0.1
i2ctransfer -f -y 30 w3@0x6b 0x04 0x0B 0x02 # Enable MIPI CSI
i2ctransfer -f -y 30 w3@0x6b 0x08 0xA0 0x84 # Force all MIPI clocks running

sleep 0.5
echo "[96712-bit3]: GMSL2 link locked"
i2ctransfer -f -y 30 w2@0x6b 0x00 0x1A r1
