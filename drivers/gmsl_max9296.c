...
static void max9296_pipes_reset(struct max9296 *priv)
{
	...
	struct pipe_ctx pipe_defaults[] = {
		{MAX9296_PIPE_X, GMSL_CSI_DT_YUV422,
			MAX9296_CSI_CTRL_1, 0, MAX9296_INVAL_ST_ID},
		{MAX9296_PIPE_Y, GMSL_CSI_DT_YUV422,
			MAX9296_CSI_CTRL_1, 0, MAX9296_INVAL_ST_ID},
...
int gmsl_max9296_power_on(struct device *dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	int err = 0;
...
EXPORT_SYMBOL(gmsl_max9296_power_on);

void gmsl_max9296_power_off(struct device *dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
...
EXPORT_SYMBOL(gmsl_max9296_power_off);
...
int gmsl_max9296_setup_link(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	int err = 0;
	unsigned int i = 0;
...
EXPORT_SYMBOL(gmsl_max9296_setup_link);

int gmsl_max9296_setup_control(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	int err = 0;
	unsigned int i = 0;
...
EXPORT_SYMBOL(gmsl_max9296_setup_control);

int gmsl_max9296_reset_control(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	int err = 0;
...
EXPORT_SYMBOL(gmsl_max9296_reset_control);

int gmsl_max9296_sdev_register(struct device *dev, struct gmsl_link_ctx *g_ctx)
{
...
EXPORT_SYMBOL(gmsl_max9296_sdev_register);

int gmsl_max9296_sdev_unregister(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = NULL;
	int err = 0;
	unsigned int i = 0;
...
EXPORT_SYMBOL(gmsl_max9296_sdev_unregister);
		...
		struct gmsl_link_ctx *g_ctx)
{
	...
	for (i = 0; i < g_ctx->num_streams; i++) {
		...
		struct reg_pair map_pipe_yuv422[] = {
			/* addr, val */
			{MAX9296_TX11_PIPE_X_EN_ADDR, 0x7},
			{MAX9296_TX45_PIPE_X_DST_CTRL_ADDR, 0x15},
			{MAX9296_PIPE_X_SRC_0_MAP_ADDR, GMSL_CSI_DT_YUV422},
			{MAX9296_PIPE_X_DST_0_MAP_ADDR, GMSL_CSI_DT_YUV422},
		...
		if (g_stream->st_data_type == GMSL_CSI_DT_YUV422) {
			map_list = map_pipe_yuv422;
			arr_sz = ARRAY_SIZE(map_pipe_yuv422);
...
int gmsl_max9296_start_streaming(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	struct gmsl_link_ctx *g_ctx;
	struct gmsl_stream *g_stream;
...
EXPORT_SYMBOL(gmsl_max9296_start_streaming);

int gmsl_max9296_stop_streaming(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	struct gmsl_link_ctx *g_ctx;
	struct gmsl_stream *g_stream;
...
EXPORT_SYMBOL(gmsl_max9296_stop_streaming);

int gmsl_max9296_setup_streaming(struct device *dev, struct device *s_dev)
{
	struct max9296 *priv = dev_get_drvdata(dev);
	struct gmsl_link_ctx *g_ctx;
	int err = 0;
...
EXPORT_SYMBOL(gmsl_max9296_setup_streaming);

static const struct of_device_id max9296_of_match[] = {
	{ .compatible = "maxim,gmsl_max9296", },
	{ },
};
...
MODULE_DESCRIPTION("Dual GMSL Deserializer driver gmsl_max9296");
